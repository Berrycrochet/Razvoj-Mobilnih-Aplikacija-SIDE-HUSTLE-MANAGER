...
public partial class AdminProfilePage : ContentPage
{
    ...
    private async void OnChangeImageClicked(object sender, EventArgs e)
    {
        try
        {
            string action = await DisplayActionSheetAsync("Izaberite Slike", "Odustani", null, "Galerija", "Kamera");
    ...
    {
        bool answer = await DisplayAlertAsync("Odjava", "Jeste li sigurni?", "Da", "Ne");
        if (answer)
